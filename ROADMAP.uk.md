...
| 3 Modular monolith | PARTIAL → R100+ | немає `app.*`; `utils.importutils` тягне Qt |
...
|---|----|--------|------------|------|--------|
| 1 | R100 | Прибрати імпорт Qt з `utils.importutils` (винести в `ui/` або DI) | Gate / тест: модуль без `ui.qt`; існуючі import-тести зелені | M | OPEN |
...
**Перший OPEN:** `R100` — прибрати Qt-імпорт з `utils.importutils`.
