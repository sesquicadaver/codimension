...
"""Bandit plugin configuration: extra command-line arguments."""
...
CONFIG_FILE = SETTINGS_DIR + "bandit.plugin.conf"
...
class BanditConfigDialog(QDialog):
    """Configuration dialog for Bandit plugin."""

    def __init__(self, parent=None):
        QDialog.__init__(self, parent)
        self.setWindowTitle("Bandit — Configuration")

        raw = " ".join(load_extra_args())
        layout = QFormLayout(self)
        self.__extraEdit = QLineEdit(self)
        self.__extraEdit.setPlaceholderText("e.g. -s B101 -x tests/")
