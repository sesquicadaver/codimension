...
| **SSH remote project open/create** | utils.ssh_remote + ui.sshprojectdlg + ui.sshbrowse | Профілі; Paramiko/Fake SFTP; R183 path containment; R184 host-key RejectPolicy + fingerprint pin + TOFU UI; R211 pin у `missing_host_key` до auth; R213 `validate_binding`; **R221** `local_root` == `remote_cache_dir(profile, remote_root)`; R185 lstat/reject symlink + nonzero caps + stream + staging swap; **R248**/**R255**/**R261** durable atomic replace (intent phases, atomic marker, remote lock, no sole-copy loss); remote Browse… | tests/test_ssh_remote_project.py; test_ssh_r248.py; test_ssh_r255.py; test_ssh_r261.py; [ssh-remote-project.md](../technology/ssh-remote-project.md) |
...
| **Update check R172 / R215 / R222** | utils.update_check + update_provenance + ui.mainmenu/mainwindow | GitHub Releases read-only; trusted HTTPS host/path; size-capped JSON; redirect hops + final `geturl()` re-validated; діалог новішого тега | tests/test_update_check.py; tests/test_update_provenance_r215.py; tests/test_update_redirect_r222.py; [release-channels.md](../technology/release-channels.md) |
...
Подальша черга: [ROADMAP.uk.md](../../ROADMAP.uk.md) — **R232–R275** ✅; хвиля **R269–R275** (`codi-last.md` @ fdb29cad) закрита.
