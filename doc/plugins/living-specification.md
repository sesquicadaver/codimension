...
| **Taint MVP R143** | core.taint | Function-local sources→sinks; R194 `heuristic`+`confidence`; **R227** `posonlyargs` + may-taint branch join; **R239** forward CFG (no whole-list re-exec; throw-point try; match no-match; loop-else); **R258** `ExitKind` / join only NORMAL (`return`/`break`/`continue`/`raise` excluded; break skips loop else) | tests/test_taint.py, test_taint_r239.py, test_taint_r258.py; [taint-mvp.md](../uk/technology/taint-mvp.md) |
...
Подальша черга: [ROADMAP.uk.md](../../ROADMAP.uk.md) — хвиля R221–R231 ✅; **R232–R266** ✅; хвиля **R261–R268** (`codi-last.md` @ f44c8dc4); Next = **R267** multi-exit taint.
