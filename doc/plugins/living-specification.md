...
| **SSH remote project open/create** | utils.ssh_remote + ui.sshprojectdlg + ui.sshbrowse | Профілі; Paramiko/Fake SFTP; runtime deps `paramiko`/`keyring`; remote Browse…; локальний кеш; меню Open/Create | tests/test_ssh_remote_project.py; [ssh-remote-project.md](../technology/ssh-remote-project.md) |
...
| **Unresolved import choice** | ui.unresolvedimportsdlg + diagram.importsdgm + importutils | Dialog: persist artifact excludes **or** pip-install selected packages; skip optional `try/except ImportError` (e.g. `native`) as pip candidates | tests/test_analysis_excludes.py, test_importutils.py |
...
Подальша черга: [ROADMAP.uk.md](../../ROADMAP.uk.md) — **активна черга порожня**; deferred R180–R182 лише за явним запитом.

### Матриця меж модулів (R103)

Імпортер → дозволені **іменовані** шари (інші пакети поза матрицею). Qt заборонено в `core` / `infrastructure` / `app`.
...
\*legacy: `utils → ui|plugins` дозволено тимчасово (GlobalData / Qt-хелпери); ціль — прибрати в наступних задачах.
