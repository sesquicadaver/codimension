...
| **AI UI R152** | core.ai_ui + core.ai_tasks + core.ai_http + core.ai_project_context + core.ai_docstring + ui.ai* + editor/Options | Analyze; Google docstring; Chat; R192 budgeted HTTP + cancel + base_url trust; **R218** versioned `DocstringTarget` (file/version/qualname/span/fingerprint) + post-patch `ast.parse` | tests/test_ai_ui.py, test_ai_tasks.py, test_ai_config.py, test_ai_http.py, test_ai_docstring_r218.py |
...
| **MCP backend R182 / R214 / R223** | mcp_backend + optional ``mcp`` SDK | stdio MCP over headless core; ``CDM_MCP_TOKEN`` fail-closed; immutable ``--workspace`` / ``CDM_MCP_WORKSPACE``; in-walk file/byte/depth budgets (``walker``); 7 MVP tools | tests/test_mcp_r182.py; tests/test_mcp_walker_r223.py; [mcp-backend.md](../../technology/mcp-backend.md) |
| **Polyglot layer R200+** | core.language/semantic/structural/bindings/dependency_edges/cross_language_nav/tasks/document_store/…; infrastructure.lsp_* + file_uri + tree_sitter_structural + ffi_bindings + ffi_structural + build_tasks; app.language_services; ui.language_controller | Registry→…→BindingIndex→typed deps/cross-nav→TaskProviders; Stage 1–4 = R200–R208; R209 lifecycle; R210 server→client; R217/R226 FFI EXACT (structural); **R224** DocumentStore; **R225** plugin manifest; **R229** caps match provider APIs; **R230** IDE workspace attach/detach | tests/test_language_r200.py … r210.py; test_language_r224.py; test_language_r206.py; test_language_r226.py; test_language_r229.py; test_language_r230.py; [polyglot-language-layer.md](../../technology/polyglot-language-layer.md); R200–R210 + R217 + R224–R226 + R229–R230 DONE |
...
Further queue: [ROADMAP.md](../../../ROADMAP.md) — **R231** (AI structured findings + global budgets); wave R221–R231 (`codi-last.md` @ 8824ff3c).
