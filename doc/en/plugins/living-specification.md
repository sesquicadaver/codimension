...
| **Utils versions R196** | utils.versions + ui.about | Qt version injected from UI; versions.py off legacy allowlist | tests/test_versions_r196.py |
...
| **SSH remote save/run** | utils.ssh_project_runtime + runmanager | R186 async Save upload + Run (cancel/timeout/output cap); SYNC_* ≠ local save; debug deferred | tests/test_ssh_project_runtime.py |
...
Further queue: [ROADMAP.md](../../../ROADMAP.md) — **R197→** (A224…); linear non-blocking.
