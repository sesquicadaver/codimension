...
class _DocstringFrag:
    """Docstring fragment compatible with cdmflowparser / flow UI.

    Flow UI (``scopeitems``, ``auxitems``, mouse navigation) expects span
    fields (``beginLine``, ``body``, …) like other fragments. CML validation
    also reads ``leadingCMLComments`` / ``sideCMLComments`` (empty for AST).
    """
    ...
    def __init__(self, text: str | None, body: _Body | None = None) -> None:
        ...
        self.body = body
        # Mirror body spans — UI reads both ``docstring.beginLine`` and
        # ``docstring.body.beginLine``.
