...
class CodeBlockCell(CellElement, TextMixin, ColorMixin, QGraphicsRectItem):
    ...
    def __init__(self, ref, canvas, x, y):
        ...
        ColorMixin.__init__(
            self,
            ref,
            self.canvas.settings.codeBlockBGColor,
    ...
    def getSelectTooltip(self):
        ...
        return "Code block at " + CellElement.getLinesSuffix(self.getLineRange())


class ReturnCell(CellElement, TextMixin, ColorMixin, IconMixin, QGraphicsRectItem):
    ...
    def __init__(self, ref, canvas, x, y):
        ...
        ColorMixin.__init__(
            self,
            ref,
            self.canvas.settings.returnBGColor,
    ...
    def paint(self, painter, option, widget):
        ...
        lineXPos = xPos + self.iconItem.iconWidth() + 2 * settings.hTextPadding
        ...
        textRect = QRectF(
            xPos + self.iconItem.iconWidth() + 3 * settings.hTextPadding + textShift,
            yPos + settings.vTextPadding,
            self.textRect.width(),
            self.textRect.height(),
        )
...
class RaiseCell(CellElement, TextMixin, ColorMixin, IconMixin, QGraphicsRectItem):
    ...
    def __init__(self, ref, canvas, x, y):
        ...
        ColorMixin.__init__(
            self,
            ref,
            self.canvas.settings.raiseBGColor,
...
class AssertCell(CellElement, TextMixin, ColorMixin, IconMixin, QGraphicsRectItem):
    ...
    def __init__(self, ref, canvas, x, y):
        ...
        ColorMixin.__init__(
            self,
            ref,
            self.canvas.settings.assertBGColor,
    ...
    def render(self):
        ...
        self.minWidth = max(
            self.textRect.width() + 2 * settings.hCellPadding + 2 * settings.hTextPadding + self.__diamondWidth,
            settings.minWidth,
        )
    ...
    def draw(self, scene, baseX, baseY):
        ...
        settings = self.canvas.settings
        self.iconItem.setPos(
            baseX + self.__diamondWidth / 2 + settings.hCellPadding - self.iconItem.iconWidth() / 2,
            baseY + self.minHeight / 2 - self.iconItem.iconHeight() / 2 + takenByBadges / 2,
        )
    ...
    def paint(self, painter, option, widget):
        ...
        painter.setFont(settings.monoFont)
        painter.setPen(pen)
        availWidth = self.minWidth - 2 * settings.hCellPadding - self.__diamondWidth
        textWidth = self.textRect.width() + 2 * settings.hTextPadding
...
class SysexitCell(CellElement, TextMixin, ColorMixin, IconMixin, QGraphicsRectItem):
    ...
    def __init__(self, ref, canvas, x, y):
        ...
        ColorMixin.__init__(
            self,
            ref,
            self.canvas.settings.sysexitBGColor,
    ...
    def getSelectTooltip(self):
        """Provides tooltip"""
        return "Sys.exit() at " + CellElement.getLinesSuffix(self.getLineRange())


class ImportCell(CellElement, TextMixin, ColorMixin, IconMixin, QGraphicsRectItem):
    ...
    def __init__(self, ref, canvas, x, y):
        ...
        ColorMixin.__init__(
            self,
            ref,
            self.canvas.settings.importBGColor,
    ...
    def paint(self, painter, option, widget):
        ...
        painter.setPen(pen)
        textRectWidth = (
            self.minWidth - 2 * settings.hCellPadding - 4 * settings.hTextPadding - self.iconItem.iconWidth()
        )
...
class DecoratorCell(CellElement, TextMixin, ColorMixin, QGraphicsRectItem):
    ...
    def draw(self, scene, baseX, baseY):
        ...
        if self.isFirst:
            ...
            self._connector = Connector(
                self.canvas, startXPos, yPos, self.baseX + settings.hCellPadding - settings.boxLineWidth, yPos
            )
            ...
            self._vDotConnector = Connector(self.canvas, self.baseX, yBeginPos, self.baseX, yEndPos)
            self._vDotConnector.penStyle = Qt.DotLine
            scene.addItem(self._vDotConnector)

            self._hDotConnector = Connector(
                self.canvas, self.baseX, yEndPos, self.baseX + settings.hCellPadding, yEndPos
            )
...
class IfCell(CellElement, TextMixin, ColorMixin, QGraphicsRectItem):
    ...
    def __init__(self, ref, canvas, x, y):
        ...
        ColorMixin.__init__(
            self,
            ref,
            self.canvas.settings.ifBGColor,
    ...
    def render(self):
        ...
        self.minHeight = self.textRect.height() + 2 * settings.vCellPadding + 2 * settings.vTextPadding
        self.minWidth = max(
            self.textRect.width() + 2 * settings.hCellPadding + 2 * settings.hTextPadding + 2 * settings.ifWidth,
            settings.minWidth,
        )
    ...
    def __calcPolygon(self, takenByBadges):
        ...
        self.x3 = baseX + self.minWidth - settings.hCellPadding - settings.ifWidth - shift
        self.y3 = self.y2
    ...
    def draw(self, scene, baseX, baseY):
        ...
        if self.needHConnector:
            ...
            self.rhsConnector = Connector(self.canvas, self.x4, self.y4, self.baseX + self.width - hShift, self.y4)
        else:
            ...
            self.rhsConnector = Connector(self.canvas, xPos, self.y5, xPos, self.baseY + self.height)
        ...
        self.leftBadge.moveTo(self.x1 - self.leftBadge.width / 2, self.y3 - self.leftBadge.height / 2)

        penWidth = settings.selectPenWidth - 1
        self.setRect(
            self.x1 - penWidth, self.y2 - penWidth, self.x4 - self.x1 + 2 * penWidth, self.y6 - self.y2 + 2 * penWidth
        )
        scene.addItem(self)
