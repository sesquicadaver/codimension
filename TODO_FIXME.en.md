...
No confirmed **P0**. **P1: 0**. **P2: 0** — wave **R269–R275** ✅; **R276** (pylint subprocess wrapt) DONE with this fix. Slice `codi-last.md` @ master@fdb29cad (Qt/QThread lifecycle) closed.
...
| P1-03 | Global `exceptionHook` → `application.exit(1)` for any uncaught callback | P1 | ✅ R272 |
...
| P1-03 | DocumentStore: URI alias (`file://localhost`) can replace `OPEN_BUFFER` with disk `DISK` | P1 | ✅ R263 |
| P1-04 | Plugin identity: symlinked directories omitted from `package_sha256` but still importable | P1 | ✅ R264 |
...
| P1-03 | Plugin package identity fail-open for oversized/unreadable member (`package_sha256=""`) | P1 | ✅ R254 + ✅ R264 |
...
| P2-03 | Project switch: unload without rollback after `before_load` | P2 | ✅ R257 + ✅ R265 |
