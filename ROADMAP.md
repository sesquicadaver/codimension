...
| 21–24 Graph | PARTIAL → R140.a+ | Headless parse exists (`core.flow` / `parsers.flow_ast`); canvas-separated graph model + UI consume still OPEN |
...
|---|----|------|------------|------|--------|
| 1 | R140.a | Headless CFG **graph model** API from flow parse (nodes/edges/spans; Qt-free; reuse `core.flow` / `flow_ast` data, not canvas) | Module + unit tests on fixtures; Living Spec; no `flowui` rewrite yet | M | OPEN |
| 2 | R140.b | `flowui` / editor canvas **consumes** the R140.a model (thin adapter; behavior parity) | Existing CF tests/smoke green; no duplicate layout source of truth | L | OPEN |
| 3 | R141 | Debugger graph mode: map frames → CFG nodes | Offscreen/debugger test or documented manual + unit mapping | L | OPEN |
| 4 | R142 | Graph diff between two CFGs / revisions | Diff API + tests on fixture pairs | M | OPEN |
| 5 | R143 | Function-local data-flow / taint MVP | Documented subset; tests for sources/sinks in one function | L | OPEN |
| 6 | R150 | Plugin capability / API version negotiation | Host rejects incompatible plugins cleanly; test | S | OPEN |
| 7 | R151 | AI context builder (headless): pack SymbolIndex + CFG slice for a symbol | Pure function + tests; no network | M | OPEN |
| 8 | R152 | AI UI actions behind feature flag (explain / suggest) | Flag off by default; smoke when flag on may mock backend | M | OPEN |
| 9 | R160 | Environment overlay visualization (env source / path badges on UI) | Uses R135; screenshot or widget test | M | OPEN |
| 10 | R161 | Dependency overlay (edge heat from DependencyGraph) | Uses R133+R135; test | M | OPEN |
| 11 | R162 | Deployment overlay hints (Dockerfile/compose detection) | Read-only hints; test on fixtures | S | OPEN |
| 12 | R171 | Release channel metadata in `cdmverspec` (`stable`/`beta`/`dev` label, still one version) | Field + docs; no multi-branch required | S | OPEN |
| 13 | R172 | In-app “check for updates” against GitHub Releases (read-only) | Shows newer tag if any; test with mocked HTTP | M | OPEN |
| 14 | R173 | Download + checksum verify update artifact | Writes to cache dir; verify fail closed; tests | M | OPEN |
| 15 | R174 | Feature flags module for experimental plugins/UI | Persistent flags; gate one existing experimental path; tests | S | OPEN |
| 16 | R175 | Safe-mode startup (disable plugins / overlays) | CLI or env `CDM_SAFE_MODE=1`; smoke | S | OPEN |
...
**First OPEN:** `R140.a` — Headless CFG graph model API (canvas consume = R140.b).
